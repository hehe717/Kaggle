...
class CardinalityBlock(nn.Module):
    def __init__(self, in_channels, out_channels, identity_downsample=None, stride=1, C=32):
        ...
        self.conv2 = nn.Conv2d(self.small_out_channels, self.small_out_channels, kernel_size=3, stride=stride,
                               padding=1)
        self.bn2 = nn.BatchNorm2d(self.small_out_channels)
        self.conv3 = nn.Conv2d(self.small_out_channels, self.small_out_channels * self.expansion, kernel_size=1,
                               stride=1, padding=0)
        ...
        self.branch = nn.Sequential(self.conv1, self.conv2, self.conv3)
...
def ResNeXt50(image_channels=3, num_classes=1000):
    WHAT = [3, 4, 6, 3] # ResNeXt50
    return ResNeXt(CardinalityBlock, WHAT, image_channels, num_classes)
